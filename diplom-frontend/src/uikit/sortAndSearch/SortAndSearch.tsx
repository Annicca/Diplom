...
export const SortAndSearch:FC<SortAndSearchProps> = ({filter, handleFilter}) => {
    ...
    const optionsTypeSort:{
        ...
        selectedType,
        {value: ETypeSort.ASC, label: 'Старые'}
    ]
    ...
    const handleTypeSort = (event: SingleValue<{
        value: ETypeSort;
        label: string;
    }>) => {
        handleFilter({
    ...
    return(
        <div className={style.sortSearch}>
            ...
            {!IS_MOBILE && <Search handleSearch={handleCity} placeholder="Введите город" />}
            <Select 
