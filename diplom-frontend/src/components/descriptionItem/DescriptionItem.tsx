...
export const DescriptionItem: FC<DescriptionItemProps> = ({ description }) => {
  ...
  return (
    <div className={style.description}>
      ...
      <div className={style.description__text} key={Number(isUnwrap)}>
        {" "}
        {isUnwrap || description.length < 100
          ? description
      ...
      {description.length > 100 && (
        <Button
          ...
          isClear={true}
          isYellow={false}
          className={style.description__btn}
        >
          <div className="text-orange">
            {isUnwrap ? " Скрыть" : " Развернуть всё"}
          </div>
        </Button>
      )}
    </div>
