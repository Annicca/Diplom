...
export const CreateGenres:FC<CreateGenresProps> = ({register, errors, control, nominationIndex}) => {

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const {fields: genresFields, append, remove} = useFieldArray({control, name: `nominations.${nominationIndex}.genres` as any})
    ...
    return(
        <>
            {genresFields.map(({id}, index) => (
                <article key = {id} className={style.genres__inner}>
                    <div className={style.genres__name}>
                        ...
                        <Button onClick={() => remove(index)} className={style.genres__button}>
                    ...
                    <InputControl 
                        type = "text" 
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        {...register(`nominations.${nominationIndex}.genres.${index}.name` as any, {
            ...
            <Button type="button" className={style.genres__add} onClick={() => append({name: ''})}>Добавить жанр</Button>
        </>
    )
}