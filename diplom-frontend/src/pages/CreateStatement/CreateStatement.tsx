...
export const CreateStatement: FC = () => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    data.regulation &&
      ...
      formData.append("regulation", data.regulation[0]);
    data.nominations &&
      data.nominations.length > 0 &&
      addObjectToFormData(formData, data.nominations, "nominations");
    data.ageCategories &&
      data.ageCategories.length > 0 &&
      addObjectToFormData(formData, data.ageCategories, "ageCategories");
    data.groupCategories &&
      data.groupCategories.length > 0 &&
      addObjectToFormData(formData, data.groupCategories, "groupCategories");
      ...
      <form onSubmit={onSubmit} className={style.form}>
        {stage === 1 && (
          <>
            ...
            <div
              className={classNames(style.radioContainer, style.formContent)}
            >
              <InputControl
                id="competition"
                type="radio"
        ...
        {stage === 2 && getValues().type === ETypeStatement.GROUP && (
          <>
            <div className={style.title}>2. Заполните данные о коллективе</div>
        ...
        {stage === 2 && getValues().type === ETypeStatement.COMPETITION && (
          <>
            <div className={style.title}>2. Заполните данные о конкурсе</div>
            <div className={style.formContent}>
              <CreateCompetition
                control={control}
                register={register}
        ...
        {stage === 3 && getValues().type === ETypeStatement.COMPETITION && (
          <>
            <div className={style.title}>3. Добавьте номинации</div>
            <div className={style.formContent}>
              <CreateNominations
                register={register}
                errors={errors}
        ...
        {stage === 4 && getValues().type === ETypeStatement.COMPETITION && (
          <>
            <div className={style.title}>4. Добавьте возрастные группы</div>
            <div className={style.formContent}>
              <CreateAgeCategories
                register={register}
                errors={errors}
        ...
        {stage === 5 && getValues().type === ETypeStatement.COMPETITION && (
          <>
            <div className={style.title}>5. Добавьте групповые формы</div>
            <div className={style.formContent}>
        ...
        {stage === 6 && getValues().type === ETypeStatement.COMPETITION && (
          <>
            <div className={style.title}>6. Проверьте введенную информацию</div>
            <div className={style.formContent}>
        ...
        {((stage === 2 && getValues().type === ETypeStatement.GROUP) ||
          ...
          <>
            ...
            >
              <Button
                onClick={
                  getValues().type === ETypeStatement.COMPETITION
                    ? () => toggleStage(5)
                    : () => toggleStage(1)
                }
                className={style.stage_btn}
